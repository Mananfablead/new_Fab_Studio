...
export default function AddFeaturesModal({ open, onOpenChange, plan, isActivePlan = false, onPaymentSuccess }: AddFeaturesModalProps) {
  ...
  const { refetch: refetchUserPlans } = useUserPlans();
  ...
  const dropdownFeatures = features.filter(
    (f) => f.addons.length > 0 && (getPayloadKey(f.feature_name) !== null || f.addons.some((a) => a.feature_value !== null))
  );

  const toggleFeatures = features.filter(
    (f) => getPayloadKey(f.feature_name) === null && (f.addons.length === 0 || (f.addons.length === 1 && f.addons[0].feature_value === null))
  );
  ...
  const handleProceedToCheckout = useCallback(async () => {
    ...
    try {
      ...
      dropdownFeatures.forEach(feat => {
        const key = getPayloadKey(feat.feature_name);
        if (key && dropdowns[key as keyof typeof dropdowns] !== 'none') {
          const opt = feat.addons.find(a => (a.feature_value || '') === dropdowns[key as keyof typeof dropdowns]);
          if (opt) {
      ...
      openRazorpayCheckout({
        ...
        onSuccess: async ({ razorpay_order_id, razorpay_payment_id, razorpay_signature }) => {
          try {
            ...
            // Critical: Update the user's plan state in Redux so they aren't blocked anymore
            refetchUserPlans();
            ...
            <div className="p-8 overflow-y-auto flex-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              <div className="flex flex-col">
                ...
                {dropdownFeatures.map((feature) => {
                  ...
                  console.log("Dropdowns", features);
                    ...
                    <div key={feature.id} className="flex items-center justify-between py-3 px-2 border-b border-slate-50 hover:bg-slate-50/50 transition-colors rounded-lg">
                      <div className="flex items-center gap-4 flex-1">
                        <button
                          ...
                          className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 border transition-all ${isSelected ? 'bg-green-100 border-transparent' : 'border-slate-300 hover:border-slate-400 bg-white'
                            }`}
                        >
                ...
                {featuresLoading ? (
                  ...
                  toggleFeatures.map((feature) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4 flex-1">
                          <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 border transition-all ${isSelected ? 'bg-green-100 border-transparent' : 'border-slate-300 group-hover:border-slate-400 bg-white'
                            }`}>
                            {isSelected && <Check className="w-3 h-3 text-green-600" />}
                          </div>
                          <input
